...
class TransformerAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        agent_args.add_argument(
            "--eval_type", type=str, default="hits@1", help="hits@1, ppl or f1"
        )
    ...
    def next_word_probability(self, partial_out):
        ...
        instance = build_input_from_segments(
            self.persona, self.history, partial_out_ids, self.tokenizer, with_eos=False
        )

        input_ids = torch.tensor(
            instance["input_ids"], device=self.args.device
        ).unsqueeze(0)
        token_type_ids = torch.tensor(
            instance["token_type_ids"], device=self.args.device
        ).unsqueeze(0)
    ...
    def get_prefix2words(self, convai_dict, smoothing_freq=5):
        ...
        for prefix_id, words in prefix2words.items():
            ...
            prefix2words[prefix_id] = dict(
                (word, count / total_counts) for word, count in words.items()
            )
